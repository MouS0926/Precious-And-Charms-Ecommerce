...
export const fetchPage:any = () => {
    return async (dispatch: Dispatch) => {
      try {
        const response = await axios.get(`https://precious-n-charms.onrender.com/products`); // Replace with your API endpoint
...
// ****************page ************
  ...
  export const fetchData:any = (page:number) => {
    return async (dispatch: Dispatch) => {
      try {
        const response = await axios.get(`https://precious-n-charms.onrender.com/products?_page=${page}&_limit=20`); // Replace with your API endpoint
  ...
  export const deleteData = (id: number):any => {
    return async (dispatch: Dispatch) => {
      try {
        await axios.delete(`https://precious-n-charms.onrender.com/products/${id}`);
        dispatch({
...
type AppThunk = ThunkAction<void, RootState, null, Action<string>>;
  export const postProduct:any=(newprod:productObj): AppThunk=>(dispatch: Dispatch)=>{
    ...
    axios.post(`https://precious-n-charms.onrender.com/products`,newprod)
    .then((res)=>{
